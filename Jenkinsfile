pipeline {
   ...
   stages {
     ...
     stage("test") {
      ...
      stage("register") {
                steps {
                  snDevOpsArtifact(artifactsPayload:"""{"artifacts": [{"name": "mbart.jar","version":"1.${env.BUILD_NUMBER}","semanticVersion": "1.${env.BUILD_NUMBER}.0","repositoryName": "mbrepo", "pipelineName": "${env.JOB_BASE_NAME}"}]}""")
